...
function OAuth2Strategy(options, verify) {
  ...
  this._grant_type = options.grant_type;
  ...
  if (options.authorizationURL) {
        ...
        this._key = options.sessionKey || ('oauth2:' + url.parse(options.tokenURL).hostname);
    }
  if (options.store) {
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  var meta = {
    authorizationURL: null,//this._oauth2._authorizeUrl,
    ...
    if (req.query && req.query.code) {
        ...
        if (scope) {
            if (Array.isArray(scope)) { scope = scope.join(this._scopeSeperator); }
            params.scope = scope;
        }
        ...
        if (state) {
            ...
            self._oauth2.getOAuthAccessToken('', params, function(err, accessToken, refreshToken, params) {
                self._loadUserProfile(accessToken, function(err, profile) {
                    ...
                    try {
                        //checks number of paramaters of verify -- assumption of parameter
